...
| LLDP | **`Enabled`**; the baseline was `Disabled`. The 1.20.4 default came up under an unchanged config, as expected. **Kept on** deliberately, for mapping neighbors port by port, and declared as `switch_lldp: true` (net #21, inventory #33). With the declaration in place, `--tags mgmt` finds the switch already matching and changes nothing. |
