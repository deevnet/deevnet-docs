...
| 00:15:27–00:17:18 | Step 8, MinIO and the API on `prv` | `site.yml --skip-tags vms --limit dv02prv001v01`: `changed=38`, `failed=0`. It created the private `deevnet-api` podman network, pushed and loaded the MinIO, PostgreSQL 17.11 and API images, built and enabled all three containers, opened 9000, 9001 and 8080 in firewalld, created the `tf-state` bucket with versioning, and wrote and loaded eds's scoped policy. eds's state credential was re-issued from the vault, not generated. |
| after 00:18 | Step 8 verify, `prv` | On the VM: all three services active, enabled and never restarted; `deevnet-api-db` and `deevnet-api` alone on the private network with PostgreSQL's 5432 unpublished; firewalld open only for 8080, 9000 and 9001; `tf-state` versioned; eds present with its `tenant-eds` policy; no failed units; 888 MiB of 3.8 GiB used. From the Builder by name: MinIO live, ready and console all 200; the API's `/healthz` 200, `/readyz` 200 with `database: ok`, `/version` v0.1.0 (commit `7d1cf36`), and `/v1` 401 with no token and with a wrong one. |
...
The change is complete; these are carried separately.
