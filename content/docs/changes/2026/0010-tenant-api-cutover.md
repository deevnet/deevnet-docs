...
| **Status** | **Planned.** Every piece is merged and nothing is deployed. |
...
**Secrets the operator generates and vaults.** Nothing here is generated by automation, and no step
proceeds without them.

**`vault_openbao_seal_key`** — `group_vars/openbao/vault.yml`

```bash
head -c 32 /dev/urandom | base64
```

**`vault_powerdns_api_key`** — `group_vars/all/vault.yml`

```bash
openssl rand -hex 32
```

**`vault_minio_deevnet_api_secret`** — `group_vars/all/vault.yml`
...
tokens: a tenant can only restore itself while this key is the same.

```bash
head -c 32 /dev/urandom | base64
```

**`vault_deevnet_egress_agent_token`** — `host_vars/dv02hyp002p02/vault.yml`, and the same value as
the API's `DEEVNET_AGENT_TOKEN` in `group_vars/deevnet_api/vault.yml`.

```bash
openssl rand -hex 32
```

**`vault_deevnet_api_proxmox_token_id` and `_secret`** — `group_vars/deevnet_api/vault.yml`. Already
issued on hv02: role `DeevnetTenantBuilder`, token `deevnet-api@pve!tenants`, with `Sys.Audit`
scoped to `/nodes` through `DeevnetNodeAudit`. Re-issuing means deleting and re-creating the token,
because Proxmox shows a secret once.

**`vault_openbao_ansible_role_id` and `_secret_id`** — `group_vars/openbao/vault.yml`. **These do not
exist yet: step 3 produces them**, in
`.openbao/dv02idn001v01-init.json` on the control node, along with the recovery key. Move both into
