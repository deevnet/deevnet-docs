...
| Value | Where | How |
|---|---|---|
| `vault_openbao_seal_key` | `group_vars/openbao/vault.yml` | `head -c 32 /dev/urandom \| base64` |
| `vault_powerdns_api_key` | `group_vars/all/vault.yml` | `openssl rand -hex 32` |
| `vault_minio_deevnet_api_secret` | `group_vars/all/vault.yml` | `openssl rand -hex 20` |
| `vault_deevnet_api_token_hmac_key` | `group_vars/deevnet_api/vault.yml` | `head -c 32 /dev/urandom \| base64` |
| `vault_deevnet_api_proxmox_token_id`, `_secret` | `group_vars/deevnet_api/vault.yml` | already issued: role `DeevnetTenantBuilder`, token `deevnet-api@pve!tenants` on hv02 |
| `vault_deevnet_egress_agent_token` | `host_vars/dv02hyp002p02/vault.yml` | `openssl rand -hex 32`; the same value is the API's `DEEVNET_AGENT_TOKEN` |
| `vault_openbao_ansible_role_id`, `_secret_id` | `group_vars/openbao/vault.yml` | **step 3 produces these**; they do not exist yet |
