...
| **Extended by** | [ADR-0013: Management-Hypervisor Services Run as Containers on Domain VMs](/docs/architecture/decisions/0013-management-services-domain-vms/) — where the controller runs (a container in the network management VM, `nms`), and the order a rebuild uses it in |
