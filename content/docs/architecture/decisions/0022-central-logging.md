...
- **The substrate has its own ingest token**, which writes only `(0, 0)`, and an operator read
  token covering every partition. Both are kept in OpenBao.
...
|---|---|---|
| Substrate hosts (Fedora VMs, hypervisors) | `systemd-journal-upload` over HTTPS to vmauth; `Header=` needs systemd 258+ | the substrate ingest token |
...
devices' addresses. That is the same split between zone policy and host policy that the broker's SSH
port already relies on.

### 6. Retention

- **One retention period for the whole store in v1.** VictoriaLogs' retention is configured per
  store. Whether it can be set per tenant is To confirm.
...
1. **Operator queries across tenants.** VictoriaLogs' documentation doesn't say whether one query
   can span several `AccountID`s. If it can't, the operator queries one partition at a time, or the
   substrate publishes a copy of tenant events into `(0, 0)` as well.
2. **Tenant UI.** VictoriaLogs' built-in UI behind vmauth, or Grafana with one organisation per
   tenant? The first needs nothing new; the second is familiar and heavier.
3. **Publishing third-party services' tenant events.** A collector such as Vector on `tob` could map
   PowerDNS zone updates and VerneMQ authentication failures to a tenant, but only if the log line
   identifies the tenant reliably.
4. **Per-tenant retention and quotas.** One tenant must not be able to fill the disk for all of them.
   Does VictoriaLogs support per-tenant limits, or does vmauth need a rate limit per token?
5. **Workload identity.** v1 gives a tenant one ingest token shared by its workloads. A per-workload
   token, delivered through ADR-0021, would allow per-workload revocation.
6. **The Builder.** It roams and belongs to no site. Should it ship to the site it is attached to?
...
- That vmauth ignores or overwrites a caller-supplied `AccountID` and `ProjectID` once `headers` sets
  them for a user.
- That `systemd-journal-upload` with `Header=` on the site's Fedora release delivers into the right
  partition through vmauth.
- That a VictoriaLogs syslog listener can be fixed to `(0, 0)` with `-syslog.tenantID.tcp`, and that
  the core router, switch and AP can send RFC 5424 to it. Check each device's syslog options against
  its current manual, not from memory.
- That VictoriaLogs and vmauth run in 2 GB with a realistic ingest rate, or what `tob` needs instead.
