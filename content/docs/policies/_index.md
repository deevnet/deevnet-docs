---
title: "📜 Policies & Procedures"
weight: 3
bookCollapseSection: true
---

# Policies & Procedures

How Deevnet is governed: how change is introduced, how failures are handled, and how risk is
identified and accepted. The [runbook](/docs/runbook/) says *how to do* a thing; this section says
*what has to be true* while doing it, and what gets written down afterwards.

The records these procedures produce live in their own sections:
[Change Records](/docs/changes/), [Incident Records](/docs/incidents/), and
[Certification](/docs/platforms/certification/) under Implementation & Tooling.

<div class="section-cards">
  <a class="section-card" href="change-management/">
    <h3>Change Management</h3>
    <p>Change classification, validation before apply, and the change record template.</p>
  </a>
  <a class="section-card" href="incident-management/">
    <h3>Incident Management</h3>
    <p>How an incident is recorded and followed through, and the incident record template.</p>
  </a>
  <a class="section-card" href="risk-management/">
    <h3>Risk Management</h3>
    <p>Vulnerabilities, security controls, traceability, resiliency, and the risk register.</p>
  </a>
  <a class="section-card" href="lifecycle-management/">
    <h3>Lifecycle Management</h3>
    <p>From selection to retirement, including certification: what hardware and software must demonstrate before they are relied on.</p>
  </a>
</div>
